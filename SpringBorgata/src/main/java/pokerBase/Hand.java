...
public class Hand {
	...
	public void JOKERandWild(){
		...
		for(Hand hand:newHand){
			hand.EvalHand();
		}
		Collections.sort(newHand, Hand.HandRank);
		
		this.setBestHand(newHand.get(0).getCards());
		this.HandStrength=newHand.get(0).getHandStrength();
		this.HiHand=newHand.get(0).getHighPairStrength();
		this.LoHand=newHand.get(0).getLowPairStrength();
		this.Kickers=newHand.get(0).getKicker();
	}
